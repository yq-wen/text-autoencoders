...
from model import DAE, VAE, AAE, DVAE
...
def evaluate(model, batches):
    ...
    with torch.no_grad():
        for inputs, targets in batches:
            if args.model_type == 'dvae':
                # TODO: This needs to be fixed:
                # should be x1, x2, y1, y2 instead
                losses = model.autoenc(inputs, inputs, targets, targets)
            else:
...
def main():
    ...
    if args.model_type == 'dvae':
        train_ref_sents = load_sent('data/quora/train/ref.txt')
    ...
    if args.model_type == 'dvae':
        valid_ref_sents = load_sent('data/quora/val/ref.txt')
    ...
    if not os.path.isfile(vocab_file):
        if args.model_type == 'dvae':
            Vocab.build(train_sents + train_ref_sents, vocab_file, args.vocab_size)
        else:
    ...
    model = {'dae': DAE, 'vae': VAE, 'aae': AAE, 'dvae' : DVAE}[args.model_type](
    ...
    if args.model_type == 'dvae':
        train_ref_batches, _ = get_batches(train_ref_sents, vocab, args.batch_size, device)
        valid_ref_batches, _ = get_batches(valid_ref_sents, vocab, args.batch_size, device)
    best_val_loss = None
    for epoch in range(args.epochs):
        ...
        print('#length', len(train_sents), len(train_ref_sents))
        ...
        for i, idx in enumerate(indices):
            ...
            if args.model_type == 'dvae':
                ref_inputs, ref_targets = train_ref_batches[idx]
                # losses = model.autoenc(inputs, inputs, targets, targets, is_train=True)
                losses = model.autoenc(inputs, ref_inputs, targets, ref_targets, is_train=True)
            else:
